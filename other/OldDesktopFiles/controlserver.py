...
def turn_left(duration=1):
    # Example: stop left motors and run right motors forward to turn left
...
def turn_right(duration=1):
    # Example: run left motors forward and stop right motors to turn right
    FRONT_left_motor_forward()
    BACK_left_motor_forward()
...
@app.route('/')
def index():
    # A simple HTML page with buttons to control the car
    return """
    <html>
    <head>
...
# --- Run Flask and Clean Up GPIO on Exit ---
if __name__ == '__main__':
    try:
        # The host is set to 0.0.0.0 so that the server is accessible from the network
